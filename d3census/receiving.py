...
from .geography import FullGeography, Geography
...
def perfect_geo_attribute(attribute_name):
    map_to_perfect = {
        "school district (unified)": "school district",
    }
    return (
        map_to_perfect.get(attribute_name, attribute_name)
        .lower()
        .replace(" ", "_")
    )
...
def filter_to_geo_attrs(dictionary):
    ...
    return {
        perfect_geo_attribute(key): value
        for key, value in dictionary.items()
        if perfect_geo_attribute(key) in set(Geography.__match_args__)
    }
...
def build_full_geography(dictionary):
    ...
    for key, value in dictionary.items():
        try:
            ...
            result.__setattr__(perfect_geo_attribute(key), value)
