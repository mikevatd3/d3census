...
def test_diff_method_same_result():
    assert create_geography("05000US26163") == create_geography(
        state=26, county=163
    )
...
def test_create_geography_mixed_inputs():
    with pytest.raises(
        ValueError,
        match="You can only provide a geoid, ucgid, or individual components, not multiple.",
    ):
...
def test_create_geography_missing_inputs():
    with pytest.raises(
        ValueError,
        match="You have to provide either a full geoid, ucgid, or individual geography components.",
    ):
        create_geography()
...
def test_create_geography_invalid_geoid_format():
    with pytest.raises(
        ValueError,
        match="'create_geography' only accepts full geoids eg. 05000US26163.",
    ):
...
def test_create_geography_invalid_parts():
    with pytest.raises(
        ValueError,
        match="The components you provided, invalid, are not valid geography parts",
    ):
...
def test_create_geography_parts_block_group():
    result = create_geography(
        state="26", county="163", tract="521400", block_group="1"
    )
    expected = Geography(
        sum_level=SumLevel.BLOCK_GROUP,
        parts={
...
def test_create_geography_parts_ucgid():
    ...
    expected = Geography(
        sum_level=SumLevel.ZCTA, parts={UCG.ID: "860Z200US15007"}, ucgid=True
    )
    assert result == expected
